...
import petAlterado from '../fixtures/pet Alterado.json'
...
describe('CRUD da entidade Pet', () => {
  ...
  it('DELETE Pet', () => {
    cy.request({
      ...
      expect(status).to.eq(200) //status code __> comunicação com a API
      expect(body.code).to.eq(200) // Process code __> processamento na API
      expect(body.type).to.eq('unknown') // Tipo desconhecido _ padrão
